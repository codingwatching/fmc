...
fn update_light(
    ...
    mesh_query: Query<
        ...
        (
            With<Handle<PbrMaterial>>,
            Or<(Changed<GlobalTransform>, Added<Handle<PbrMaterial>>)>,
        ),
    >,
...
fn replace_material_and_mesh(
    ...
    for (entity, standard_handle, mesh_handle) in material_query.iter() {
        ...
        let pbr_handle = pbr_materials.add(PbrMaterial::from(standard_material));
        let mut entity_commands = commands.entity(entity);
        entity_commands.remove::<Handle<StandardMaterial>>();
        entity_commands.insert(pbr_handle);
        let mesh = meshes.get(mesh_handle).unwrap().clone();
        let new_mesh_handle = meshes.add(mesh);
        entity_commands.insert(new_mesh_handle);
    }
}
