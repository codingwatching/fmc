...
use crate::{
    ...
    world::{
        blocks::Blocks,
        world_map::{chunk::ChunkFace, NewChunkEvent, WorldMap},
        Origin,
    },
};
            ...
            for i in 0..CHUNK_SIZE {
                for j in 0..CHUNK_SIZE {
                    let (index, adjacent_index) = match chunk_face {
                        ...
                        ChunkFace::Top => (i << 8 | j << 4 | (CHUNK_SIZE - 1), i << 8 | j << 4),
                        ChunkFace::Bottom => (i << 8 | j << 4, i << 8 | j << 4 | (CHUNK_SIZE - 1)),
                        ChunkFace::Right => ((CHUNK_SIZE - 1) << 8 | i << 4 | j, i << 4 | j),
                        ChunkFace::Left => (i << 4 | j, (CHUNK_SIZE - 1) << 8 | i << 4 | j),
                        ChunkFace::Front => (i << 8 | (CHUNK_SIZE - 1) << 4 | j, i << 8 | j),
                        ChunkFace::Back => (i << 8 | j, i << 8 | (CHUNK_SIZE - 1) << 4 | j),
...
impl LightChunk {
    fn new_normal() -> Self {
        Self {
            is_sunlit: false,
            light: LightStorage::Normal(vec![Light::new(0, 0); CHUNK_SIZE.pow(3)]),
        }
    }
    ...
    fn convert_to_normal(&mut self) {
        if matches!(self.light, LightStorage::Uniform(_)) {
            self.light = LightStorage::Normal(vec![Light::new(0, 0); CHUNK_SIZE.pow(3)]);
        }
    }
...
impl LightUpdateQueue {
    fn new() -> Self {
        Self {
            ...
            propagation: VecDeque::with_capacity(CHUNK_SIZE.pow(2) * 6),
            removal: Vec::new(),
        }
    }
...
fn propagate_light(
    ...
    for chunk_position in light_update_queues.keys().cloned().collect::<Vec<IVec3>>() {
            ...
            let light = match &mut light_chunk.light {
                LightStorage::Uniform(uniform_light) => {
                    if uniform_light.sunlight() != 0 {
                        light_chunk.light =
                            LightStorage::Normal(vec![*uniform_light; CHUNK_SIZE.pow(3)]);
        ...
        if update_queue.propagation.len() > 0 && update_queue.sunlit {
            ...
            let mut height_map: [usize; CHUNK_SIZE.pow(2)] = [CHUNK_SIZE; CHUNK_SIZE.pow(2)];
            ...
            for x in 0..CHUNK_SIZE {
                for z in 0..CHUNK_SIZE {
                    ...
                    for (shift_x, shift_z) in [(1, 0), (0, 1), (-1, 0), (0, -1)] {
                        ...
                        // outside the chunk.
                        if shifted_x >= CHUNK_SIZE || shifted_z >= CHUNK_SIZE {
