...
// Interface nodes that are interacted with are referenced by their path in the ui hierarchy.
// The path is formatted "parent/child/grandchild" by the names given in the config. For example an
// inventory may have a root node named "inventory" and a child named "equipment", yielding
// "inventory/equipment". When interacting with an interface e.g. moving items, this name is sent
// to the server to identify which interface and which section is being interacted with.
#[derive(Component)]
...
pub fn load_interfaces(
    ...
    for dir_entry in directory {
        ...
        fn spawn_interface(
            ...
            let interface_path = if let Some(name) = &config.name {
                let interface_path = if parent_path == "" {
                    name.to_owned()
                } else {
                    parent_path + "/" + name
                };

                entity_commands.insert(InterfacePath(interface_path.clone()));
                interfaces.insert(interface_path.clone(), entity_commands.id());
        ...
        commands
            ...
            .with_children(|parent| {
                ...
                let interface_config = InterfaceRootConfig {
...
fn cursor_visibility(
    ...
    changed_interfaces: Query<(&Visibility, &InterfaceRootConfig), Changed<Visibility>>,
) {
...
fn button_interaction(
    ...
    for (interaction, interface_path) in button_query.iter() {
        match *interaction {
            Interaction::Pressed => net.send_message(messages::InterfaceButtonPress {
...
fn handle_visibility_updates(
    ...
    mut interface_query: Query<&mut Visibility, With<InterfacePath>>,
    ...
    for visibility_updates in visibility_update_events.read() {
        for (interface_path, new_visibility) in visibility_updates.updates.iter() {
            let interface_entity = match interfaces.get(interface_path) {
                Some(i) => *i,
                None => {
                    net.disconnect(&format!(
                        "Server sent a visibility update for the interface: '{}', but there is no interface by that name.",
                        &interface_path
                    ));
                    return;
                }
...
fn handle_toggle_events(
    ...
    mut interface_query: Query<(Entity, &mut Visibility, &InterfaceRootConfig)>,
...
fn handle_escape_key(
    ...
    mut interface_query: Query<(&mut Visibility, &InterfaceRootConfig)>,
