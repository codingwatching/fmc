...
use fmc_networking::{
    ...
    BlockId, NetworkClient, NetworkData,
};
use serde::{Deserialize, Serialize};

use crate::{assets::models::Models, game_state::GameState, world::blocks::Blocks};

use super::{InterfacePath, Interfaces};
...
pub fn load_items(
    ...
    for (filename, id) in server_config.item_ids.iter() {
        ...
        let model_id = match models.get_id_by_filename(&json_config.equip_model) {
            Some(id) => id,
            None => {
        ...
        if gltf.named_animations.get("left_click").is_none() {
            net.disconnect(&format!(
                "Misconfigured resource pack: missing animation 'left_click' for model '{}'\
                    , needed for item '{}'",
                &json_config.equip_model, &json_config.name,
            ));
            return;
        }
...
fn return_cursor_item(
    net: Res<NetworkClient>,
    items: Res<Items>,
    visibility_changed: Query<(), (Changed<Visibility>, With<InterfacePath>)>,
    ...
    if !cursor_box.is_empty() {
        for (item_box_section, children, visibility, interface_path) in
