...
impl F64GlobalTransform {
    ...
    pub fn from_translation(translation: Vec3) -> Self {
        F64GlobalTransform(Affine3A::from_translation(translation))
    }

    #[doc(hidden)]
    #[inline]
    pub fn from_rotation(rotation: Quat) -> Self {
        F64GlobalTransform(Affine3A::from_rotation_translation(rotation, Vec3::ZERO))
    }

    #[doc(hidden)]
    #[inline]
    pub fn from_scale(scale: Vec3) -> Self {
        F64GlobalTransform(Affine3A::from_scale(scale))
    }
    ...
    /// will be invalid.
    #[inline]
    pub fn compute_transform(&self) -> F64Transform {
    ...
    #[inline]
    pub fn radius_vec3a(&self, extents: Vec3A) -> f64 {
    ...
    #[inline]
    pub fn mul_transform(&self, transform: F64Transform) -> Self {
        Self(self.0 * transform.compute_affine())
    }
}

impl Default for F64GlobalTransform {
    fn default() -> Self {
        Self::IDENTITY
    }
}

impl From<F64Transform> for F64GlobalTransform {
    fn from(transform: F64Transform) -> Self {
        Self(transform.compute_affine())
    }
}

impl From<Affine3A> for F64GlobalTransform {
    fn from(affine: Affine3A) -> Self {
        Self(affine)
    }
}

impl From<Mat4> for F64GlobalTransform {
    fn from(matrix: Mat4) -> Self {
        Self(Affine3A::from_mat4(matrix))
    }
}

impl Mul<F64GlobalTransform> for F64GlobalTransform {
    type Output = F64GlobalTransform;

    #[inline]
    fn mul(self, global_transform: F64GlobalTransform) -> Self::Output {
        F64GlobalTransform(self.0 * global_transform.0)
    }
}

impl Mul<F64Transform> for F64GlobalTransform {
    type Output = F64GlobalTransform;

    #[inline]
    fn mul(self, transform: F64Transform) -> Self::Output {
        self.mul_transform(transform)
    }
}

impl Mul<Vec3> for F64GlobalTransform {
    type Output = Vec3;
