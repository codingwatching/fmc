...
macro_rules! get_3d_noise {
    ($setting:expr) => {
        if is_x86_feature_detected!("avx2") {
            // There seems to be some problem here. Avx2 is very slow(20x) compared to sse2. Many
            // on the bugtracker also seem to have problems with stack overflows. Tried to
            // diagnose, but I don't understand anything. Seems to be something wrong in the second
            // loop of the get_3d_noise_helper macro.
